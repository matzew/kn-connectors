...
[source,text]
----
quay.io/openshift-knative/kn-connector-aws-ddb-streams-source:1.0-SNAPSHOT
----
...
[source,shell]
----
kubectl set env deployment/kn-connector-aws-ddb-streams-source CAMEL_KAMELET_{{KAMELET_NAME}}_{{PROPERTY_NAME}}="I updated it..."
----
...
Given a configmap named `my-aws-ddb-streams-source-config` in Kubernetes that has two entries:

.my-aws-ddb-streams-source-config
[source,properties]
----
...
spec:
  selector:
    matchLabels:
      app.kubernetes.io/name: aws-ddb-streams-source
      app.kubernetes.io/version: 1.0-SNAPSHOT
  template:
    spec:
      containers:
        - image: localhost:5001/openshift-knative/aws-ddb-streams-source:1.0-SNAPSHOT
...
spec:
  ...
  template:
    spec:
      ...
      volumes:
        - configMap:
            name: my-aws-ddb-streams-source-config
          name: kn-source-config
----
...
* _ce-type_: dev.knative.connector.event.aws-ddb-streams
* _ce-source_: dev.knative.eventing.aws-ddb-streams-source
* _ce-subject_: aws-ddb-streams-source
...
[source,shell]
----
kubectl set env deployment/kn-connector-aws-ddb-streams-source KN_CONNECTOR_CE_OVERRIDE_TYPE=custom-type
----
