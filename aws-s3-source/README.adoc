...
[source,text]
----
quay.io/openshift-knative/kn-connector-aws-s3-source:1.0-SNAPSHOT
----
...
[source,shell]
----
kubectl set env deployment/kn-connector-aws-s3-source CAMEL_KAMELET_{{KAMELET_NAME}}_{{PROPERTY_NAME}}="I updated it..."
----
...
Given a configmap named `my-aws-s3-source-config` in Kubernetes that has two entries:

.my-aws-s3-source-config
[source,properties]
----
...
spec:
  selector:
    matchLabels:
      app.kubernetes.io/name: aws-s3-source
      app.kubernetes.io/version: 1.0-SNAPSHOT
  template:
    spec:
      containers:
        - image: localhost:5001/openshift-knative/aws-s3-source:1.0-SNAPSHOT
...
* _ce-type_: dev.knative.connector.event.aws-s3
* _ce-source_: dev.knative.eventing.aws-s3-source
* _ce-subject_: aws-s3-source
...
[source,shell]
----
kubectl set env deployment/kn-connector-aws-s3-source KN_CONNECTOR_CE_OVERRIDE_TYPE=custom-type
----
